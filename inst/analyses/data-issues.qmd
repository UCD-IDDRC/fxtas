...
Primary ethnicity we can drop. 
Primary race we may be able to look on Randi's cards for that or ask her. 
There are only 44 to look up.
...
Age of onset and missingness reasons where "head tremor hx" = "Yes":
...
table1(na.is.category = FALSE, 
  ...
  stratified_formula(vars, column_var),
  # render.continuous = c(.="N", .="Mean (SD)", .="Median [Min,  Max]"),
  data = visit1 |> dplyr::filter(Study == "GP4" & `head tremor hx` == "Yes"),
  footnote = footnote)
...
n_age_NoHeadTremor <- visit1 |> 
    dplyr::filter((`head tremor hx` == "No" |
             is.na(`head tremor hx`)) & 
             Study == "GP4" &
...
table1(na.is.category = FALSE, 
  ...
  stratified_formula(vars, column_var),
  # render.continuous = c(.="N", .="Mean (SD)", .="Median [Min,  Max]"),
  data = visit1 |> filter(CGG < 200))

```

```{r}
v1_usable |> 
  filter(ataxia == "Yes", `ataxia severity` == 0) |> 
  select("FXS ID", "Event Name", contains("ataxia")) |> 
  relocate(sort(tidyselect::peek_vars())) |> 
  relocate(c("FXS ID", "Event Name"), .before = everything())
```
