...
sessioninfo::package_info(pkgs = "installed") |> 
  ...
  readr::write_csv("inst/analyses/packages.csv")
```
...
if (FALSE) {
  ...
  rcartocolor::carto_pal(name = "Safe") |> 
    colorblindcheck::palette_plot(show_hex = TRUE)
  rcartocolor::carto_pal(name = "Safe")[scale_color_indices] |>
    colorblindcheck::palette_plot(show_hex = TRUE)
  rcartocolor::carto_pal(name = "Safe", n = 8) |>
    colorblindcheck::palette_plot(show_hex = TRUE)
...
# note: don't install miniconda on DEM mac laptop; it breaks the current setup
...
abbrevs_caption <- abbreviations |> 
  build_abbrevs_caption(abbreviations_to_define = abbrevs_list1)

abbrevs_list2 <- abbrevs_list1 |> union(c("Hyp.", "WM"))

abbrevs_caption2 <- abbreviations |> 
  build_abbrevs_caption(abbreviations_to_define = abbrevs_list2)
...
biomarker_levels2 <- biomarker_levels |> create_blevels_no_abbrev()
...
sustain_output_path <- fs::path(
  output_folder,
  "sustain_output.rds"
)

if (fit_models || !file.exists(sustain_output_path)) {
  ...
  sustain_output <- run_and_save_OSA(
    ...
    readr::write_rds(file = sustain_output_path)
} else {
  sustain_output <- readr::read_rds(file = sustain_output_path)
}
```
...
abstract_text <- paste0(
  ...
  "Fragile X-associated tremor/ataxia syndrome (FXTAS) patients. "
)
...
fig_both_first2 |> 
  ggplot2::ggsave(
    ...
    units = "mm", 
    dpi = 600,
    filename = "inst/analyses/thumbnail.jpeg"
  )
```


```{r}
#| label: extract-strat-results
...
(Supplementary material, Detecting latent subtypes)
...
The out-of-fold log-likelihood ("OOFLL") 
(Supplementary material, Detecting latent subtypes) 
...
type_0_summary <- glue::glue(
  "{n_type_0} carriers had experienced too few events ",
  "to be accurately classified into a subtype ",
  "and were excluded from these results"
)
...
subtype_pval_summaries <- glue::glue(
  ...
  "(N = {N_subtyped} premutation carriers",
  # " who could be clustered into a subtype", # nolint
  ";\n",
  "{type_0_summary})."
)
```
...
```{r}
#| results: asis
#| label: code-tbl_sg_demos
tbl_sg_demos |>
  gtsummary::modify_footnote_header(
    columns = stat_0,
    footnote = 
      glue::glue("n (column %); {type_0_summary}.")
  ) |> 
...
```{r}
#| label: thumbnail-by-sex

fig_females_first2 <- results_females_first |> 
  plot_positional_var(
    biomarker_groups = biomarker_groups,
    biomarker_levels = biomarker_levels2,
    use_rds = use_rds,
    size.y = size.y,
    n_s = 1,
    use_labels = TRUE
...
fig_males_first2 <- results_males_first |> 
  plot_positional_var(
    biomarker_groups = biomarker_groups,
    biomarker_levels = biomarker_levels2,
    use_rds = use_rds,
...
abstract_text <- paste0(
  ...
  "is a neurodegenerative disorder affecting adult fragile X gene premutation carriers. ",
  "An open question is: in what sequential order do FXTAS symptoms appear, ",
  "and does the sequence vary between males and females? ",
  "We used Ordinal Subtype and Stage Inference ",
  "to show sex differences in symptom onset order, ",
  "highlighting the need for sex-specific diagnostic and staging criteria."
) # |> stringr::str_wrap()

pvdc_sex2 <- list(
  "Males:" = fig_males_first2,
  "Females:" = fig_females_first2
) |>
  plot_compact_pvd(
    scale_colors = scale_colors1, 
    y_text_size = 8,
    legend.position = "right",
    legend.direction = "vertical",
    legend.key.height = grid::unit(.35, "lines"),
    show_group_color_legend = FALSE,
...
pvdc_sex2 |> 
  ggplot2::ggsave(
    ...
    units = "mm", 
    dpi = 600,
    filename = "inst/analyses/graphical-abstract.pdf"
  )
...
**Positional differences in estimated event sequence between males and females.** `r compact_fig_cap2` `r test_sex_summary`
