...
#'
#' @return a tibble with the same columns as `dataset`,
#' except with the following additions:
#' * FX (logical): TRUE if CGG >= 55
#' - `FX*` (factor): dichotomizes CGG to < 55 vs >= 55;
#' `NA`s stay as `NA`
#' - `FX**` (factor): dichotomizes CGG to < 55 vs >= 55;
#' `NA`s converted to level `"CGG missing"`
#' - `FX3*` (factor):
#' trichotomizes CGG to < 55, 55-100, and >100 >= 55;
#' `NA`s stay as NAs
#' - `FX3**` (factor):
#' trichotomizes CGG to < 55, 55-100, and >100 >= 55;
#' `NA`s converted to level "CGG missing"
  ...
  dataset |>
    dplyr::mutate(
      ...
      `FX3*` =
        case_when(
          .data$`CGG` < 55 ~ "CGG <55",
          .data$`CGG` |> between(55, 99) ~ "CGG 55-99",
          .data$`CGG` |> between(100, 199) ~ "CGG 100-199",
          .data$`CGG` >= 200 ~ "CGG \u2265 200",
          .ptype = factor(
            levels = c(
              "CGG <55",
              "CGG 55-99",
