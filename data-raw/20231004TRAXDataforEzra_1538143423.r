...
label(data$mds_ne_pfprt) <- "pill-rolling tremor exam"
...
label(data$mds_neu_atax_age) <- "ataxia age of onset hx"
label(data$mds_neu_atax_sev) <- "ataxia severity hx"
...
label(data$new_mds_ne_ga) <- "ataxia exam"
...
label(data$new_mds_ne_pfprt) <- "pill-rolling tremor exam"
label(data$new_mds_ne_pfsg) <- "Stiff gait"
label(data$new_mds_neu_atax) <- "ataxia hx"
...
levels(data$dem_edlev.factor) <- c("K-7", "8-9", "10-11", "High School/GED", "Partial College", "BA/BS", "MA/MS/PhD/MD", "No Response (999)")
