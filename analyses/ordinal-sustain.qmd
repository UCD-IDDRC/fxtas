...
biomarker_text_color_info <- 
  glue::glue(
    "Y-axis label text colors indicate {outcomes_name} categories ",
    "(@tbl-biomarker-list)."
  )
...
```{r}
permutation_test_summary <- function(
    test_stat,
    pval,
    N = nrow(v1_usable_cases)) {
...
test_sex_summary <- pval_sex_gp34_v1 |> summary(N = nrow(v1_usable_cases))
...
@suppfig-pvd-by-gender-cgg_over100-1).
...
biomarkers_table |> 
  left_join(
    biomarker_groups |> distinct(biomarker_group, group_color),
...
The first level listed in each row of the
